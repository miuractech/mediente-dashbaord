...
interface SimpleTaskListComponentProps {
  projectId: string;
...
export function SimpleTaskListComponent({ projectId, onTaskEdit, onTaskClick, statusFilter, showOnlyCompleted }: SimpleTaskListComponentProps) {
  ...
  const filters = useMemo((): TaskFilters => {
    const finalFilters: TaskFilters = {
      ...
      is_loaded: true, // Only show currently loaded tasks
    };
  ...
  // Listen for global task updates to refresh this component
  useGlobalTaskRefresh(projectId, handleSilentRefresh); // Use silent refresh for global updates

  // Check if current step is completed (for UI feedback only)
  const currentStepTasks = useMemo(() => 
    tasks.filter(t => t.is_loaded && !t.is_custom), [tasks]);
  
  const allCompleted = useMemo(() => 
    currentStepTasks.length > 0 && 
    currentStepTasks.every(t => t.task_status === 'completed'), 
    [currentStepTasks]
  );
  ...
  const handleEditTask = (task: ProjectTaskWithAssignments) => {
  ...
  return (
    <Stack gap="md">
      ...
      {!statusFilter && !showOnlyCompleted && (
      ...
      {tasks.length === 0 ? (
        <Center h={200}>
          <Text c="dimmed">{showOnlyCompleted ? 'No completed tasks found' : 'No tasks found in current step'}</Text>
        </Center>
      ) : (
        <Stack gap="sm">
          {tasks.map((task) => (
      ...
      {allCompleted && (
        <Alert color="green" variant="light">
          <Group justify="space-between">
            <Text>
              🎉 All tasks in this step are completed! Ready to load the next step.
            </Text>
            <Button
              size="sm"
              variant="light"
