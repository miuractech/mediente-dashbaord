...
export function PaginatedTaskListComponent({ 
  ...
  // Fetch upcoming tasks for table view overview
  const upcomingFilters = useMemo((): TaskFilters => ({
    project_id: projectId,
  ...
  const { 
    tasks: upcomingTasks 
  } = useProjectTasks(upcomingFilters);

  // Limit upcoming tasks to 3
  const limitedUpcomingTasks = useMemo(() => 
      ...
      {!loading && tasks.length === 0 ? (
        ...
        <Stack gap="sm">
          {tasks.map((task) => (
            ...
            {/* Add upcoming tasks section for card view when showUpcoming is true */}
            {showUpcoming && limitedUpcomingTasks.length > 0 && (
              <>
                {/* Divider */}
                <Paper withBorder p="sm" style={{ backgroundColor: 'var(--mantine-color-blue-0)' }}>
                  <Text size="sm" fw={500} ta="center" c="dimmed">
                    Next Upcoming Tasks (Preview)
                  </Text>
                </Paper>
                ...
                {limitedUpcomingTasks.map((task) => (
                  <Card 
                    key={`upcoming-${task.project_task_id}`} 
                    withBorder 
                    p="md"
                    style={{ 
            ...
            <Table striped highlightOnHover>
              <Table.Thead>
                <Table.Tr>
                  ...
                  <Table.Th>Assigned Crew</Table.Th>
              ...
              <Table.Tbody>
                {tasks.map((task) => (
                  ...
                  >
                    ...
                    <Table.Td>
                      {task.assigned_crew.length > 0 ? (
                        <Group gap={4}>
                          {task.assigned_crew.slice(0, 2).map((crew, index) => (
                            <Tooltip key={index} label={`${crew.crew_name} (${crew.role_name})`}>
                              <Avatar size="xs" radius="xl">
                                <IconUser size={10} />
                              </Avatar>
                    ...
                    <Table.Td>
                      {task.deadline ? (
                        ...
                        >
                          ...
                          {isOverdue(task.deadline, task.task_status) && (
                    ...
                    <Table.Td>
                      {task.checklist_items.length > 0 ? (
                        <Group gap="xs">
                          <Progress
                            value={Math.round((task.checklist_items.filter(item => item.completed).length / task.checklist_items.length) * 100)}
                            size="sm"
                            w={60}
                          />
                          <Text size="xs" c="dimmed">
                ...
                {/* Add upcoming tasks section for table view when showUpcoming is true */}
                {showUpcoming && limitedUpcomingTasks.length > 0 && (
                  <>
                    {/* Section divider row */}
                    <Table.Tr style={{ backgroundColor: 'var(--mantine-color-gray-0)' }}>
                      <Table.Td colSpan={7} py="sm">
                        <Text size="sm" fw={500} ta="center" c="dimmed">
                          Next Upcoming Tasks (Preview)
                        </Text>
                      </Table.Td>
