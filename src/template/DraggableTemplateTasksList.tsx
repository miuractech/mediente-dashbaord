...
// Interface for template tasks list (all tasks from all phases)
interface DraggableTemplateTasksListProps {
...
// Template tasks list component segregated by phases and steps
export function DraggableTemplateTasksList({
  tasks,
  onReorder,
  onEdit,
  onDelete,
  ...
  templateId,
  onRoleUpdate,
}: DraggableTemplateTasksListProps) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      if (!validateMove(active.id as string, over.id as string)) {
        // Show error notification or handle invalid move
        return;
      }
  ...
  const getParentTaskName = (parentTaskId?: string) => {
    ...
    if (parentTask) {
      return `${parentTask.task_name} (${parentTask.phase_name} → ${parentTask.step_name})`;
    }
  ...
  // Group tasks by phase and step
  const tasksByPhaseAndStep = tasks.reduce((acc, task) => {
    ...
    if (!acc[phaseKey].steps[stepKey]) {
      acc[phaseKey].steps[stepKey] = {
    ...
    acc[phaseKey].steps[stepKey].tasks.push(task);
    return acc;
  }, {} as Record<string, { 
    phaseName: string; 
    phaseOrder: number; 
    steps: Record<string, { stepName: string; stepOrder: number; tasks: typeof tasks }> 
  }>);

  // Sort phases and steps by order
  const sortedPhases = Object.values(tasksByPhaseAndStep)
    .sort((a, b) => a.phaseOrder - b.phaseOrder)
    .map(phase => ({
      ...phase,
      steps: Object.values(phase.steps).sort((a, b) => a.stepOrder - b.stepOrder)
    }));
    ...
    <div style={{ overflowX: 'auto', width: '100%' }}>
      ...
      >
        ...
        <Table.Tbody>
          ...
          >
            {sortedPhases.map(({ phaseName, phaseOrder, steps }) => (
              <React.Fragment key={`phase-${phaseOrder}-${phaseName}`}>
                {/* Phase header row */}
                <Table.Tr style={{ backgroundColor: '#e3f2fd' }}>
                  <Table.Td colSpan={12}>
                    <Text fw={700} size="lg" c="blue">
                      Phase {phaseOrder}: {phaseName} ({steps.reduce((total, step) => total + step.tasks.length, 0)} tasks)
                    </Text>
                  </Table.Td>
                </Table.Tr>
                
                {/* Steps and their tasks */}
                {steps.map(({ stepName, stepOrder, tasks: stepTasks }) => (
                  <React.Fragment key={`step-${phaseOrder}-${stepOrder}-${stepName}`}>
                    {/* Step header row */}
                    <Table.Tr style={{ backgroundColor: '#f8f9fa' }}>
                      <Table.Td colSpan={12}>
                        <Text fw={600} size="md" c="indigo" pl="md">
                          Step {stepOrder}: {stepName} ({stepTasks.length} tasks)
                        </Text>
                      </Table.Td>
                    </Table.Tr>
