...
// Interface for segregated phase-step tasks list
interface DraggablePhaseStepTasksListProps {
...
export function DraggablePhaseStepTasksList({
  tasks,
  onReorder,
  onEdit,
  onDelete,
  ...
  const tasksByStep = tasks.reduce((acc, task) => {
    ...
    acc[stepKey].tasks.push(task);
    return acc;
  }, {} as Record<string, { stepName: string; stepOrder: number; tasks: typeof tasks }>);

  // Sort steps by order
  const sortedSteps = Object.values(tasksByStep).sort((a, b) => a.stepOrder - b.stepOrder);
    ...
    <div style={{ overflowX: 'auto', width: '100%' }}>
      ...
      >
        ...
        <Table.Tbody>
          ...
          >
            {sortedSteps.map(({ stepName, stepOrder, tasks: stepTasks }) => (
              <React.Fragment key={`step-${stepOrder}-${stepName}`}>
                {/* Step header row */}
